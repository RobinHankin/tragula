...
the editor.  Referees' confidentiality is absolute and will be
respected (although referees may, at their discretion, waive anonymity
in the interests of a better publication).  It is strongly suggested
that software be presented as a working \proglang{R} package, which is
expected to pass \code{R CMD check} cleanly.  Decisions will be made
in the light of reviews and rebuttals if appropriate.  The typical
flow is as follows:
