...
\Abstract{This document is part of \code{guidelines.zip}, available on
  the journal website, \url{https://ojs.stir.ac.uk/tragula}.  It
  outlines the academic and typesetting conventions for \tragula, and
  gives some guidelines for reviewers.  Authors should download
  \code{guidelines.zip} and use file \code{guidelines.Rnw} therein as
  ...
  nutshell: \tragula\ is a peer-reviewed journal that showcases novel
...
\Address{
  ...
  Scotland\\
  email: \email{tragula@stir.ac.uk}\\
  ...
  email: \email{vvoojagig@maxi.ac.ursa}\\

  Dan Streetmentioner\\  %% Dan is an independent researcher, no affiliation
}
...
\tragula\ is fully open-access, meaning articles are free to read on
...
mathematics.  The sample issue of \tragula\ contains some manuscripts
...
documentation: the ideal is a structured, heuristic narrative.  If
your package already contains a vignette, then converting it to a
\tragula\ submission is easy, and should take less than 60 minutes.

\begin{itemize}
\item Strip out the boilerplate: remove standard setup code or basic
  installation instructions (not needed for \tragula.)
  
\item Focus the narrative: highlight the motivation for the package,
  the design patterns, and the underlying data structures, and
  possibly the salient parts of the underlying \proglang{R} or
  \proglang{C++} implementation
  
\item Formatting: ensure headings and code snippets match
  \tragula\ typesetting guidelines.
\end{itemize}
...
The snippet format is particularly suitable in cases where an
\proglang{R} package implements an algorithm that already has a
peer-reviewed article.  One might hold that writing a
\tragula\ article is thus superfluous.  However, this mindset treats
software as a passive by-product of a published article.
\tragula\ takes the view that value lies in the gap between a theory
paper and the \proglang{R} package code.  A theoretical paper explains
the mathematics to statisticians---but a \tragula\ article explains
the software architecture to \proglang{R} developers, and gives a way
to highlight and showcase their software.
...
If the theoretical methodology for the submission has been published
previously, that is OK.  The value of the submission lies in the
implementation and how it operates in the wider ecology of the
\proglang{R} world.
...
\proglang{OJS} 3.5.0.3.  The standard workflow flowchart given
...
which is managed separately via email correspondence with the author.
Usually, authors should submit at least the following documents:
