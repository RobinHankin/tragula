...
\title{Guidelines for Tragula: help for authors, editors, and reviewers}
...
\Abstract{This document outlines the academic and typesetting
  ...
  Authors should use file \code{author\_guidelines.Rnw} as a starting
...
most recent version of \emph{Writing \proglang{R} extensions}.
...
\proglang{Sweave} on \code{Rnw} files facilitates this.
...
scholarly publication.  As such, authors are expected to include a
...
more straightforwardly using \proglang{RStudio}.  It is a good idea to
use this file (\verb+article_template.Rnw+) as a starting point, and
gradually replace the content with your own work.
...
advice.  I would like to thank Achim Zeileis again for his help and
encouragement.

Processing \tragula\ documents imports three PDF figures:
...
\verb+\author+ and \verb+title+ are unchanged from JSS; they are
largely self-evident but the JSS style guide gives some more technical
information.

Citation management must be via BibTeX; most of the time \verb+\citet+
or \verb+\citep+ will be appropriate, but macros \verb+\citealt+,
\verb+\citealp+, \verb+\citeauthor+, \verb+\citeyear+ give greater
control over punctuation.  A good citation might be: ``see
\citet{lowry1951} for an example''.  Subsequent citations of
many-author articles should be abbreviated as in ``\citet{lowry1951}
provide details''.  Packages (whether or not on \proglang{CRAN})
should be indicated using the \verb+\pkg+ macro, as in ``the \pkg{mvp}
package~\citep{mvp2018}''.  Citing of software can be problematic but
try to follow the citation guidelines provided by the software itself
(in the case of \proglang{R} packages, \code{citation("pkgname")}
gives the preferred BibTeX entry).

Inline equations should use dollars, as in $y=x^2$; but if an equation
is too high (e.g., a fraction or complicated integral), use display
mode.  All displayed equations must be numbered and labelled, for ease
of review, and use the \code{equation} environment, as in:
...
circle than usual for academic journals.  Here we offer some guidance
to reviewers who may not have been part of the formal peer-review
process before.
...
software and the editor might solicit your opinion as any one of:
\begin{itemize}
...
\begin{itemize}
  \item Does the submission match your area of expertise?  Editors
    ...
    constructive review.
  \item Is there either a real or perception of a conflict of
...
Respond to the invitation as soon as you can (even you decide to
decline); a delay in your slows down the review process and means more
...
in submissions being accepted or rejected.  
...
Some questions to consider might include the following:
...
The review has one of three outcomes: Accept, Revise, Reject.

\begin{description}
  \item[Accept] Makes a sizable contribution to mathematical software;
    well-written and structured; coherent; software maintanable,
    clear, novel and interesting.  It is fine to recommend acceptance,
    yet include suggestions for a better submission such as typos and
    clarifications.  The suggestions should be small in terms of
    effort and structure.
\item[Revise] Has the potential to move to {\em accept} if the authors
  fix the issues.  Well-defined and isolatable faults that have a
  ready fix.
\item[Reject] Critical problems which have no clear fix; severely
  sub-standard; patent nonsense; identical or similar software
  available elsewhere
\end{description}
...
them to get a feel for the quality of the submission.

Reviews should be critical in the sense that they involve careful
informed judgement.  But criticism in the sense of passing
unfavourable feedback or fault-finding is also a necessary part of the
review process, so do not feel the need to be overly polite or to
protect the feelings of the authors.  In particular, if you do not
understand some part of the submission, do not feel embarrassed to say
so: remember that you are an expert in the field, and editors will
find such candid comments valuable.  Authors often find that succinct
and well-placed feedback from reviewers can catalyse much improved
writing and programming.  One important component of the peer review
process is anonymity: your identity will not be revealed to the
authors (or anyone else) so you can speak freely.
...
authors.  Before sending a review, go through it and delete
...
The submission process is managed via email but a more formal system
may be introduced in the future.  Submissions are made in the first
...
reviewers, one of whom can be the editor.  Referees' confidentiality
is absolute and will be respected (although referees may, at their
discretion, waive anonymity in the interests of a better publication).
It is strongly suggested that software be presented as a working
...
\item If in scope, submission passed to section editor.
\item Reviews solicited and collated.
\item Section editor chooses one of the following:
  \begin{itemize}
  \item Accept: submission passed to typesetting and hosting
  \item Revise and resubmit: reviews passed to author for
    consideration.  Author responds to the comments with a
    point-by-point rebuttal (note that providing a git commit hash is
    an extremely effective way of supporting this).
  \item Reject.
  \end{itemize}
...
editorial board membership can be found at \url{http://www.aut.ac.nz}.
...
authors {\em rebuttal}.  This is a point-by-point response to the
reviews.  Authors should consider the rebuttal as an opportunity to
show the editors that any issues raised by the reviews have been dealt
with.  Editors might, at their discretion, ask for guidance from
reviewers but generally it is expected that editors will be able to
assess the rebuttal themselves.  It is important that the whole review
is included in the rebuttal so the editor can easily see the response
to all the comments.  Rebuttals can be provided in PDF form, or ascii.
A typical rebuttal might look like this:
...
\begin{verbatim}
   ...
   > 
     ...
     https://github.com/RobinHankin/lorentz/commit/b55e1db4


   > The documentation for function bar() is confusing and incomplete,
   > specifically it gives no examples and I could not understand how
   > the function was intended to be used.

     The vignette contains multiple informative use-cases.
     Nevertheless we have included more documentation in foo.Rd:

     https://github.com/RobinHankin/lorentz/commit/c9fd8b22
   ...
   > desirable.
     ...
     we have included a brief discussion of the Smithification in "Further
     work" section at the end.
\end{verbatim}
}

Observe how the reviewer makes comments ranging from individual typos
right through to overarching views on the entirety of the manuscript.
Authors should either accommodate the comments, or give a good reason
why not.  See how the rebuttal includes commit hashes which enable
editors to evaluate any patches.  If the manuscript itself is under
version control, sometimes it makes sense to quote the commit hash,
sometimes not.
...
possibly the manuscript will be modified post-acceptance.  The
...
\tragula\ article and the mutable vignette; and states, low-key, that
