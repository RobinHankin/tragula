...
\Abstract{Abstract goes here, not more than a few sentences.  You can
  follow ISO 214:1976 if you wish, no obligation.  Do not include
  citations here.  In a nutshell: {\em Tragula} is a peer-reviewed
  journal that showcases novel mathematical applications of
  \proglang{R}}
...
\verb+\documentclass[article]{tragula}+
  
Generally, a submission will be an \verb+.Rnw+ file which may be
processed using \proglang{Sweave} to produce a \verb+.tex+ file.  It
is a good idea to use this file (\verb+article_template.Rnw+) as a
starting point, and gradually replace the content with your own work.

The \LaTeXe{} document class \pkg{tragula} is a (slight, cosmetic)
modification of the \pkg{jss} document class, used for publications in
the Journal of Statistical Software (JSS,
\url{http://www.jstatsoft.org/}; see the ``instructions to authors''
tab and the \code{jss.pdf} style guide).

I would like to thank Achim Zeileis again for his help and
encouragement.  For {\em Tragula}, follow \pkg{jss} typesetting
requirements, with one exception: do not capitalize unless absolutely
