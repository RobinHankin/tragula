...
preferably maintained under version control such as \proglang{git}.
Reproducibility is required, and using \proglang{Sweave} on \code{Rnw}
...
tab).  I would like to thank Achim Zeleis again for his help and
...
necessary; use sentence case everywhere, including headings.

Citation management must be via BibTeX; use \verb+\citet+ or
\verb+\citep+ as appropriate.  For example, ``see \citet{lowry1951}
for details''.  Subsequent citations of many-author articles should be
abbreviated as in ``\citet{lowry1951} provide details''.  Packages
...
member of the editorial board.  The software and manuscript will be
reviewed together by at least two reviewers, one of whom can be the
editor.  Referees confidentiality is absolute and will be respected
(although the referee can waive anonymity if in the interests of a
better publication).  It is strongly suggested that software be
...
reviews and possibly rebuttals.  The typical flow is as follows:
...
\item Section editor chooses one of the following:
  ...
  \item Reject.
  \end{itemize}
\item Repeat point (4) as necessary.
\end{enumerate}
