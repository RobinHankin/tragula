...
\Abstract{This document outlines the academic and typesetting
  conventions for {\em Tragula}.  Use file
  \code{article\_template.Rnw} as a starting point for a {\em Tragula}
  submission; gradually replace the content with your own work.  The
  abstract goes here, not more than a few sentences.  You can follow
  ISO 214:1976 if you wish, no obligation.  Do not include citations
  here.  Editorial policy in a nutshell: {\em Tragula} is a
...
Good submissions to {\em Tragula} showcase novel software that
...
{\em Tragula} is an inclusive publication and welcomes submissions
...
A good {\em Tragula} submission bears many similarities to an
...
I would like to thank Achim Zeileis again for his help and
encouragement.  For {\em Tragula}, the typesetting are generally those
of JSS, with one exception: do not capitalize unless absolutely
necessary.  Use sentence case everywhere, including headings.  New
sentences are indicated with {\em two} spaces; use \verb+\colon+
...
gives the preferred \proglang{BibTeX} entry).
...
member of the editorial board.  {\em Tragula} review cannot be
...
modifications.  Tragula papers are ``static'', that is, the accepted
...
a Tragula article includes a vignette.  This vignette should be based
on the Tragula article, although it might include extra material.  A
good way to do this is to use the \proglang{jss} headers (use the the
\code{nojss} option: ``\verb+\documentclass[nojss]{jss}+''; an example
may be found at \url{https://github.com/RobinHankin/permutations}).
In the vignette, cite the Tragula article along with the other
references.  This device maintains a distinction between the static
Tragula article and the mutable vignette; and states, low-key, that
the work has been rigorously peer-reviewed.

Post-acceptance, you should include pointers to the Tragula article in
the \proglang{R} package documentation: include a reference to the
article in the \verb+\references{}+ section of any appropriate
